...
namespace ProCharts.Controls
{
    public class WaffleChart : ChartBase
    {
        public static readonly StyledProperty<IEnumerable?> ItemsSourceProperty =
            AvaloniaProperty.Register<WaffleChart, IEnumerable?>(nameof(ItemsSource));

        public static readonly StyledProperty<string?> ValuePathProperty =
            AvaloniaProperty.Register<WaffleChart, string?>(nameof(ValuePath));

        public static readonly StyledProperty<string?> TitlePathProperty =
            AvaloniaProperty.Register<WaffleChart, string?>(nameof(TitlePath));

        public static readonly StyledProperty<string?> ColorPathProperty =
            AvaloniaProperty.Register<WaffleChart, string?>(nameof(ColorPath));

        public static readonly StyledProperty<double> CellSpacingProperty =
            AvaloniaProperty.Register<WaffleChart, double>(nameof(CellSpacing), 3.0);

        public static readonly StyledProperty<double> CellCornerRadiusProperty =
            AvaloniaProperty.Register<WaffleChart, double>(nameof(CellCornerRadius), 3.0);
        ...
        public WaffleChart()
        {
            ItemsSourceProperty.Changed.AddClassHandler<WaffleChart>((x, e) => x.InvalidateVisual());
            ValuePathProperty.Changed.AddClassHandler<WaffleChart>((x, e) => x.InvalidateVisual());
            TitlePathProperty.Changed.AddClassHandler<WaffleChart>((x, e) => x.InvalidateVisual());
            ColorPathProperty.Changed.AddClassHandler<WaffleChart>((x, e) => x.InvalidateVisual());
            CellSpacingProperty.Changed.AddClassHandler<WaffleChart>((x, e) => x.InvalidateVisual());
            CellCornerRadiusProperty.Changed.AddClassHandler<WaffleChart>((x, e) => x.InvalidateVisual());
        }
        ...
        protected override void RenderChart(DrawingContext context)
        {
            ...
            var items = new List<(string Title, double Value, IBrush? Brush)>();
            int idx = 0;
            ...
            for (int row = 0; row < 10; row++)
            {
                for (int col = 0; col < 10; col++)
                {
                    ...
                    IBrush? cellBrush = Brushes.Transparent;
                    ...
                    {
                        cellBrush = new SolidColorBrush(Color.Parse("#1E293B"), 0.3); // Empty gray track
                    }
                    ...
                    if (_hoveredCategoryIndex != -1)
                    {
                        ...
                        {
                            cellBrush = new SolidColorBrush(Color.FromArgb(40, 148, 163, 184)); // Muted slate gray
                        }
                    }

                    var borderPen = new Pen(new SolidColorBrush(Color.Parse("#20FFFFFF")), 1.0);
                    context.DrawRectangle(cellBrush, borderPen, new RoundedRect(animatedRect, new CornerRadius(cornerR)));
                }
            }
        }
