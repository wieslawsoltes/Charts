...
namespace ProCharts.Avalonia.Series
{
    ...
    {
        public static readonly StyledProperty<double> BoxWidthPercentProperty =
            AvaloniaProperty.Register<BoxPlotSeries, double>(nameof(BoxWidthPercent), 0.5);

        public double BoxWidthPercent
        {
            get => GetValue(BoxWidthPercentProperty);
            set => SetValue(BoxWidthPercentProperty, value);
        }

        public BoxPlotSeries()
        {
            BoxWidthPercentProperty.Changed.AddClassHandler<BoxPlotSeries>((x, e) => x.RaiseSeriesChanged());
        }
        ...
        public override void RenderSeries(in SeriesRenderContext context)
        {
            ...
            var boxPen = new Pen(boxStroke, strokeThickness);
            ...
            foreach (var m in metricsList)
            {
                ...
                context.DrawingContext.DrawLine(boxPen, ptMin, ptQ1);
                context.DrawingContext.DrawLine(boxPen, ptQ3, ptMax);

                // Whisker horizontal end-ticks
                double tickW = boxWidth * 0.4;
                context.DrawingContext.DrawLine(boxPen, new Point(ptMin.X - tickW, ptMin.Y), new Point(ptMin.X + tickW, ptMin.Y));
                context.DrawingContext.DrawLine(boxPen, new Point(ptMax.X - tickW, ptMax.Y), new Point(ptMax.X + tickW, ptMax.Y));
                ...
                context.DrawingContext.DrawRectangle(boxFill, boxPen, boxRect);

                // Draw Median Line (horizontal line inside the box)
                context.DrawingContext.DrawLine(boxPen, new Point(ptMedian.X - boxWidth / 2.0, ptMedian.Y), new Point(ptMedian.X + boxWidth / 2.0, ptMedian.Y));
                ...
                foreach (var outl in m.Outliers)
                {
                    ...
                    context.DrawingContext.DrawEllipse(outlierBrush, null, ptOutl, 3.0, 3.0);
                }
            }
        }
    }
