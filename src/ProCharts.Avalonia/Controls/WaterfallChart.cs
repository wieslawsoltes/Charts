...
namespace ProCharts.Avalonia.Controls
{
    public class WaterfallChart : ChartBase
    {
        public static readonly StyledProperty<IEnumerable?> ItemsSourceProperty =
            AvaloniaProperty.Register<WaterfallChart, IEnumerable?>(nameof(ItemsSource));

        public static readonly StyledProperty<string?> CategoryPathProperty =
            AvaloniaProperty.Register<WaterfallChart, string?>(nameof(CategoryPath));

        public static readonly StyledProperty<string?> ValuePathProperty =
            AvaloniaProperty.Register<WaterfallChart, string?>(nameof(ValuePath));

        public static readonly StyledProperty<string?> IsTotalPathProperty =
            AvaloniaProperty.Register<WaterfallChart, string?>(nameof(IsTotalPath));

        public static readonly StyledProperty<IBrush?> PositiveBrushProperty =
            AvaloniaProperty.Register<WaterfallChart, IBrush?>(nameof(PositiveBrush));

        public static readonly StyledProperty<IBrush?> NegativeBrushProperty =
            AvaloniaProperty.Register<WaterfallChart, IBrush?>(nameof(NegativeBrush));

        public static readonly StyledProperty<IBrush?> TotalBrushProperty =
            AvaloniaProperty.Register<WaterfallChart, IBrush?>(nameof(TotalBrush));
        ...
        public string? IsTotalPath
        {
            get => GetValue(IsTotalPathProperty);
        ...
        {
            get => GetValue(TotalBrushProperty);
            set => SetValue(TotalBrushProperty, value);
        }

        public WaterfallChart()
        {
            ItemsSourceProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
            CategoryPathProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
            ValuePathProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
            IsTotalPathProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
            PositiveBrushProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
            NegativeBrushProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
            TotalBrushProperty.Changed.AddClassHandler<WaterfallChart>((x, e) => x.InvalidateVisual());
        }
        ...
        protected override void RenderChart(DrawingContext context)
        {
            ...
            var connectorPen = new Pen(new SolidColorBrush(Color.Parse("#64748B")), 1.0)
            {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                // Draw column
                context.DrawRectangle(fillBrush, borderPen, barRect);

                // Draw connector line from previous column
                if (lastBarCorner.HasValue)
                {
                    context.DrawLine(connectorPen, lastBarCorner.Value, new Point(x, yStart));
                }
