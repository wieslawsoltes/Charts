...
namespace ProCharts.Avalonia.Styles
{
    public class Palette
    {
        public string Name { get; }
        public IReadOnlyList<IBrush> Brushes { get; }

        public Palette(string name, IEnumerable<IBrush> brushes)
        {
            Name = name;
            Brushes = new List<IBrush>(brushes).AsReadOnly();
        }
        ...
        public IBrush GetBrush(int index)
        {
            if (Brushes.Count == 0)
                return Brushes.Count > 0 ? Brushes[0] : SolidColorBrush.Parse("#2196F3");
