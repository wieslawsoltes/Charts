...
namespace ProCharts.Uno.Controls
{
    ...
    /// </summary>
    public partial class GanttChart : ChartBase
    {
        ...
        public static readonly DependencyProperty TasksProperty =
            DependencyProperty.Register(nameof(Tasks), typeof(IList<GanttTask>), typeof(GanttChart), new PropertyMetadata(null, OnTasksChanged));
        ...
        {
            get => (IList<GanttTask>?)GetValue(TasksProperty);
        ...
        {
            if (d is GanttChart chart)
            {
                if (e.OldValue is System.Collections.Specialized.INotifyCollectionChanged oldColl)
                {
                    oldColl.CollectionChanged -= chart.OnTasksCollectionChanged;
                }
                if (e.NewValue is System.Collections.Specialized.INotifyCollectionChanged newColl)
                {
                    newColl.CollectionChanged += chart.OnTasksCollectionChanged;
                }

                chart.SubscribeToTasks(e.OldValue as IEnumerable<GanttTask>, e.NewValue as IEnumerable<GanttTask>);
        ...
        private void OnTasksCollectionChanged(object? sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            if (e.OldItems != null)
            {
                foreach (GanttTask item in e.OldItems)
                {
                    if (item != null) item.Changed -= OnTaskChanged;
                }
            }
            if (e.NewItems != null)
            {
                foreach (GanttTask item in e.NewItems)
                {
                    if (item != null) item.Changed += OnTaskChanged;
                }
            }
        ...
        private void SubscribeToTasks(IEnumerable<GanttTask>? oldTasks, IEnumerable<GanttTask>? newTasks)
        {
            if (oldTasks != null)
            {
                foreach (var task in oldTasks)
                {
                    if (task != null) task.Changed -= OnTaskChanged;
                }
            }
            if (newTasks != null)
            {
        ...
        protected override void RenderChart(DrawingContext context)
        {
            ...
            for (int i = 0; i < activeTasks.Count; i++)
            {
                ...
                var solidBase = baseBrush as SolidColorBrush ?? new SolidColorBrush(Microsoft.UI.Colors.DodgerBlue);
                var fillBg = new SolidColorBrush(Color.FromArgb(40, solidBase.Color.R, solidBase.Color.G, solidBase.Color.B));
                var fillProgress = baseBrush;

                if (task == _hoveredTask)
                {
                    fillProgress = new SolidColorBrush(Color.FromArgb(220, solidBase.Color.R, solidBase.Color.G, solidBase.Color.B));
                }

                // Render Background Bar
                var borderPen = new Pen(new SolidColorBrush(Color.Parse("#30FFFFFF")), 1.0);
                context.DrawRectangle(fillBg, borderPen, new RoundedRect(taskRect, new CornerRadius(4.0)));
                ...
                if (progressWidth > 0)
                {
                    ...
                    context.DrawRectangle(fillProgress, null, new RoundedRect(progressRect, new CornerRadius(4.0)));
                }
