...
namespace ProCharts.Uno.Controls
{
    ...
    /// </summary>
    public partial class BubbleMapChart : ChartBase
    {
        ...
        public static readonly DependencyProperty BubblesProperty =
            DependencyProperty.Register(nameof(Bubbles), typeof(IList<MapBubbleItem>), typeof(BubbleMapChart), new PropertyMetadata(null, OnBubblesChanged));
        ...
        {
            get => (IList<MapBubbleItem>?)GetValue(BubblesProperty);
        ...
        private static void OnBubblesChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is BubbleMapChart chart)
            {
                if (e.OldValue is System.Collections.Specialized.INotifyCollectionChanged oldColl)
                {
                    oldColl.CollectionChanged -= chart.OnBubblesCollectionChanged;
                }
                if (e.NewValue is System.Collections.Specialized.INotifyCollectionChanged newColl)
                {
                    newColl.CollectionChanged += chart.OnBubblesCollectionChanged;
                }

                chart.SubscribeToBubbles(e.OldValue as IEnumerable<MapBubbleItem>, e.NewValue as IEnumerable<MapBubbleItem>);
                chart.InvalidateVisual();
            }
        }

        private void OnBubblesCollectionChanged(object? sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            if (e.OldItems != null)
            {
                foreach (MapBubbleItem item in e.OldItems)
                {
                    if (item != null) item.Changed -= OnBubbleChanged;
                }
            }
            if (e.NewItems != null)
            {
                foreach (MapBubbleItem item in e.NewItems)
                {
                    if (item != null) item.Changed += OnBubbleChanged;
                }
            }
        ...
        private void SubscribeToBubbles(IEnumerable<MapBubbleItem>? oldItems, IEnumerable<MapBubbleItem>? newItems)
        {
            if (oldItems != null)
            {
        ...
        private void OnBubbleChanged(object? sender, EventArgs e)
        {
            InvalidateVisual();
        }
        ...
        protected override void RenderChart(DrawingContext context)
        {
            ...
            context.DrawRectangle(oceanBrush, null, area);
            ...
            for (int i = 0; i < Bubbles.Count; i++)
            {
                ...
                var solidFill = fillBrush as SolidColorBrush ?? new SolidColorBrush(Microsoft.UI.Colors.OrangeRed);

                // Overlay high opacity glowing center and transparent halo
                var haloColor = Color.FromArgb(70, solidFill.Color.R, solidFill.Color.G, solidFill.Color.B);
                var centerColor = Color.FromArgb(220, solidFill.Color.R, solidFill.Color.G, solidFill.Color.B);
